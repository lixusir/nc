...
<ul class="nav nav-list">
            ...
            <div class="page-content">
                ...
                <div class="row maintop">
                    <div class="col-xs-12 col-sm-2">
                        <a href="/qyadmin.php/Member/member_add">
                    ...
                    <div class="col-xs-12 col-sm-3" style="display:None;">
                        <form name="admin_list_sea" class="form-search" method="post" action="/admin.php?m=user&a=member_list">
                    ...
                    <div class="input-group-btn" style="display:None;">
                        <a href="/qyadmin.php/Member/member_list">
                            ...
                            <form id="leftnav" name="leftnav" method="post" action="" >
                                ...
                                <table width="100%" class="table table-striped table-bordered table-hover" id="dynamic-table">
									...
									<?php if(is_array($info)): foreach($info as $key=>$v): ?><tr>
                                            ...
                                            <td><?php echo ($v["upxianid"]); ?></td>
                                            <td><?php echo ($v["regtime"]); ?></td>
                                            <td><?php echo ($v["lognum"]); ?></td>
                                            <td>
                                        <a class="red" href="javascript:;" onclick="return stateyes(<?php echo ($v["id"]); ?>,<?php echo ($v["status"]); ?>);">
                                                <div id="zt<?php echo ($v["id"]); ?>">
												<button class="btn btn-minier <?php if($v['status']==0){echo 'btn-danger';}else{echo 'btn-yellow';} ?>"><?php if($v['status']==1){echo '账号开启';}else{echo '账号停用';} echo ($v["status"]); ?></button></div>
                                            </a>
                                                                                    </td>
                                        <td>
                                            <div class="hidden-sm hidden-xs action-buttons">
                                                <a class="green" href="/admin.php?m=user&a=member_list_edit&id=<?php echo ($v["id"]); ?>" title="修改">
                                                    <i class="ace-icon fa fa-pencil bigger-130"></i>
                                                </a>                                                
                                                <a class="blue" href="/admin.php?m=user&a=member_tree&id=<?php echo ($v["id"]); ?>" title="会员关系">
    ...
    <script>
        ...
        layer.close(index);
        window.location.href = "/admin.php?m=user&a=member_list_del&id=" + id + "";
        });
        }

        function stateyes(val,stas){
        $.post('/admin.php?m=user&a=member_list_edit&act=2',
        {x:val,y:stas},
                function(data){
                var $v = val;
                if (data.status){
                if (data.info == '账号停用'){
                var a = '<button class="btn btn-minier btn-danger">账号停用</button>'
                        $('#zt' + val).html(a);
                layer.alert(data.info, {icon: 5});
                } else{
                var b = '<button class="btn btn-minier btn-yellow">账号开启</button>'
                        $('#zt' + val).html(b);
