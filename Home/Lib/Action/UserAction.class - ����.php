...
class UserAction extends CommonAction {
	...
	public function buyCommodity(){
		...
		if($id==10){
			$hongzao=$num*150;$putao=$num*150;			
		   if($info['hongzao']<$hongzao || $info['putao']<$putao){
			$arr['reTips']='Error';$arr['reMsg']='库存不足';
			}else{
			$data['hongzao']=$info['hongzao']-$num*150;
			$data['putao']=$info['putao']-$num*150;
			...
			if($id==11){
			$hetao=$num*150;$shiliu=$num*150;			
		   if($info['hetao']<$hetao || $info['shiliu']<$shiliu){
			$arr['reTips']='Error';$arr['reMsg']='库存不足';
			}else{
			$data['hetao']=$info['hetao']-$num*150;
			$data['shiliu']=$info['shiliu']-$num*150;
            ...
            if($id==18){
			$hamigua=$num*225;$xiangli=$num*225;			
		    if($info['hamigua']<$hamigua || $info['xiangli']<$xiangli){
			$arr['reTips']='Error';$arr['reMsg']='库存不足';
			}else{
			$data['hamigua']=$info['hamigua']-$num*225;
			$data['xiangli']=$info['xiangli']-$num*225;
			...
			$data['hetao']=$info['hetao']-$num*100;
			$data['ptgouliang']=$info['ptgouliang']+$num;			
			if($db->where($where)->save($data)){
			$arr['reTips']='success';$arr['reMsg']='购买成功，获得普通狗粮'.$num.'包';	
			}
		     }
			}	
            if($id==25){//高级狗粮
			...
			$data['shiliu']=$info['shiliu']-$num*100;
			$data['gaojigouliang']=$info['gaojigouliang']+$num;			
			if($db->where($where)->save($data)){
			$arr['reTips']='success';$arr['reMsg']='购买成功，获得高级狗粮'.$num.'包';	
			}
		     }
			}	
            if($id==26){//攻击狗粮
			...
			if($db->where($where)->save($data)){
			$arr['reTips']='success';$arr['reMsg']='购买成功，获得攻击狗粮'.$num.'包';	
			}
		     }
			}	
            if($id==27){//防御狗粮
			...
			if($db->where($where)->save($data)){
			$arr['reTips']='success';$arr['reMsg']='购买成功，获得防御狗粮'.$num.'包';	
			}
		     }
			}	
            if($id==28){//速度狗粮
			...
			if($db->where($where)->save($data)){
			$arr['reTips']='success';$arr['reMsg']='购买成功，获得速度狗粮'.$num.'包';	
			}
		     }
			}		
            if($id==29){//幸运狗粮
			...
			if($db->where($where)->save($data)){
			$arr['reTips']='success';$arr['reMsg']='购买成功，获得幸运狗粮'.$num.'包';	
			}
		     }
			}	
            if($id==30){//生命狗粮
			...
			if($db->where($where)->save($data)){
			$arr['reTips']='success';$arr['reMsg']='购买成功，获得生命狗粮'.$num.'包';	
			}
		     }
			}
	...
	public function gainCrops(){
	 ...
	 if($info['fengshousx']==1){$sfj2=cfg_data(config_bumper_count);}else{$sfj2=0;}	//丰收神像+产量
		...
		$gs_type=$info['land_gs'];
		$gs_num=$chan-$jian1-$jian2-$jian3+$sfj;
	...
	public function boxCost(){
		$db = M('sysinfo');
		$info=$db->order('time desc')->limit(1)->select();
		$arr['msg']='石榴 200 个 红枣 100 个 ';
	...
	public function feedingMemberPet(){//宠物喂养
		...
		if($choseFoods=='comDogFood'){
			...
			if($info['gaojigouliang']<$foodsCount){
			   ...
			   //每袋狗粮加多少体力和经验值
			   $jiatili=rand($info2['lvl']*3,$info2['lvl']*10);
			   $jiajinyan=rand($info2['lvl']*2,$info2['lvl']*3);
			   $data['ptgouliang']=$info['ptgouliang']-$foodsCount;
			   $a=$db->where($where)->save($data);
			   $data2['tili']=$info2['tili']+$foodsCount*$jiatili;
			   $data2['jinyan']=$info['jinyan']+$foodsCount*$jiajinyan;
	...
	public function trainingMemberPet(){
		...
		if($info['tili']<$cost_tili){
			$arr['msg']='体力值不足';			
				...
				if($info['xinyun']>50){
					$data['xinyun']=$info['xinyun']-$attak-$defense-$shudu;
				}else{
					$data['xinyun']=$info['xinyun']+2;
				}
				if($db->where($where)->save($data)){
					$arr['msg']='训练成功';
				}
	...
	public function memberToRecharge(){//免费换钻石2000,只能换一次
		...
		if($rechargeType=='first'){
			...
			$data['zs']=$info['zs']+20000;
			$data['gold']=$info['gold']-200;
			$data2['userid']=session('uid');
			$data2['username']=session('username');
		...
		if($rechargeType=='third'){
			...
			$data['zs']=$info['zs']+200000;
			$data['gold']=$info['gold']-2000;
			$data2['userid']=session('uid');
			$data2['username']=session('username');
    ...
    public function getMemberNowPetInfo() {//宠物自动收获技能，持续时间	
	 ...
	 //echo time_cha(autoshou_kttime);exit;
		...
		$arr['pet_rose_heart']=$info2['nickname'];
    ...
    public function memberBuildLevelUp() {
		...
		if(I('param.getLevel')=='none'){//房屋升级
			if($info['muban']<$info['lvl']*120 || $info['shitou']<$info['lvl']*40 || $info['zs']<$info['lvl']*15000){
				$arr['code']='0';
				$arr['msg']='仓库材料不足';
			}else{
				...
				$data['muban']=$info['muban']-$info['lvl']*120;
				$data['shitou']=$info['shitou']-$info['lvl']*40;
				$data['zs']=$info['zs']-$info['lvl']*15000;
			...
			if($info['shiliu']<150 || $info['hetao']<150 || $info['zs']<200){
				$arr['code']='0';
				$arr['msg']='仓库材料不足';
			}else{
				$data['shiliu']=$info['shiliu']-150;
				$data['hetao']=$info['hetao']-150;
				$data['zs']=$info['zs']-200;
		  ...
		  if(I('post.getLevel')==3){//土地升级3
			if($info['hongzao']<225 || $info['putao']<225 || $info['zs']<300){
				$arr['code']='0';
				$arr['msg']='仓库材料不足';
			}else{
				$data['hongzao']=$info['hongzao']-225;
				$data['putao']=$info['putao']-225;
				$data['zs']=$info['zs']-300;
		...
		if(I('post.getLevel')==4){//土地升级4
			if($info['hamigua']<300 || $info['xiangli']<300 || $info['zs']<500){
				$arr['code']='0';
				$arr['msg']='仓库材料不足';
			}else{
				$data['hamigua']=$info['hamigua']-300;
				$data['xiangli']=$info['xiangli']-300;
				$data['zs']=$info['zs']-500;
